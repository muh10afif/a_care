...
<div class="page-breadcrumb">
    <div class="row">
        <div class="col-12 align-self-center">
            <h4 class="page-title">Dokumen Asset Kelolaan <?= ucwords(strtolower($nama_deb)) ?></h4>
            <div class="d-flex align-items-center">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
						<li class="breadcrumb-item"><a href="<?= base_url() ?>">Assets Care</a></li>
						<li class="breadcrumb-item">Dokumen Asset</li>
						<li class="breadcrumb-item active" aria-current="page"><a href="<?= base_url('agunan') ?>">Debitur Kelolaan</a></li>
                    </ol>
                </nav>
            </div>
        </div>
		...
		<div class="col-md-12">
			<div class="card shadow">
					...
					<div class="btn-group">
						...
						<div class="dropdown-menu animated flipInX">
							<?php foreach ($jenis_asset as $j): ?>
								<li><a  class="dropdown-item" href="<?= base_url("agunan/tambah_agunan/".$id_debitur.'/'.$j['id_jenis_asset']) ?>"><?= ucwords($j['jenis_asset']) ?></a></li>
							<?php endforeach ?>
						</div>
					</div>
				...
				<div class="card-body table-responsive">
					<table class="table table-bordered table-hover tabel" id="tabel_non" width="100%">
	            			...
	            			<?php $no=1; foreach ($asset as $a): ?>
	            				<tr>
		            				...
		            				<td align="center">
		            					...
		            					<?php endif ?>
										<a href="<?php echo base_url() ?>katalog/detail_asset/<?= $a['id_jenis_asset'] ?>/<?= $a['id_debitur'] ?>/<?= $a['id_dokumen_asset'] ?>/det_debitur"><button class="btn btn-sm waves-effect waves-light btn-outline-info" type="button">Detail</button>
										</a></td>
		            			</tr>
	            			<?php $no++; endforeach ?>
	            		</tbody>
  ...
  $(document).ready(function () {
    ...
    $('#pilih_asset').on('click', function () {
        ...
        swal({
            ...
            if (result.value) {
                $.ajax({
                    url         : "<?= base_url("agunan/ubah_status_non_debitur/$id_debitur") ?>",
                    method      : "POST",
                    beforeSend  : function () {
                        swal({
                            title   : 'Menunggu',
