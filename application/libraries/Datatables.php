      ...
      if($sSearch != '')
        for($i = 0; $i < count($mColArray); $i++)
          if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns))
            if($this->check_cType())
              $sWhere .= "CAST(".$this->select[$mColArray[$i]['data']] . " AS VARCHAR) LIKE '%" . strtoupper($sSearch) . "%' OR ";
            else
              $sWhere .= "CAST(". $this->select[$this->columns[$i]] . " AS VARCHAR) LIKE '%" . strtoupper($sSearch) . "%' OR ";
